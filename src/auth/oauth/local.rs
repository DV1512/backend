    ...
    };
    {
        ...
        match data.0 {
            ...
            TokenRequest::Password { username, password } => {
                let db_query = format!("SELECT *, count() as count FROM user WHERE email = \"{}\" AND array::any(<-auth_for<-user_auth, |$a| !type::is::none($a.password) AND type::is::string($a.password) AND $a.password==\"{}\") FETCH auth;", username, password);
